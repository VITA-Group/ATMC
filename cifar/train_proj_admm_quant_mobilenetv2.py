...
if args.abc_special:
    ...
    if not args.lr_initialize:
        ...
        model = CLlr(ranks=ranks_up)
        model.load_state_dict(torch.load(model_path))
    modelu, modelz = CLlr(ranks=ranks_up), CLlr(ranks=ranks_up)
