...
import dataset
from mlp.mlp_dense import MLP as CLdense
from mlp.mlp_abcv2 import MLP as CLabcv2
from mlp.mlp_abcv2 import MLP as CLlr
...
parser.add_argument('--attack_iter', type=int, default=16, help="attack iteration for the adversarial sample computation")
...
if args.abc_special:
    ...
    if not args.lr_initialize:
        ...
        fname = os.path.join(args.loaddir, args.model_name[0:-4] + ".npy")
        ranks_up = np.save(fname)
    else:
        fname = os.path.join(args.loaddir, args.model_name[0:-4] + ".npy")
        ranks_up = np.load(fname)
        model = CLlr(ranks_up)
        print('++++++++++++++++++++++')
        print('load model from %s' % model_path)
        print('++++++++++++++++++++++')
...
try:
    ...
    if quantize_algo is not None:
        if args.quantize_algo == "kmeans_nnz_fixed_0_center":
            ...
            weight_dict_size = [ (item - 1) * 32 for item in list(pt.layers_unique(model, weight_name)[1].values())] 
            ...
            weight_dict_size = [item * 32 for item in list(pt.layers_unique(model, weight_name)[1].values())]
        elif args.quantize_algo == "kmeans_fixed_nnz":
            weight_bits = [math.ceil(math.log2(item-1)) for item in list(pt.layers_unique(model, weight_name)[1].values())]
            weight_dict_size = [ (item - 1) * 32 for item in list(pt.layers_unique(model, weight_name)[1].values())] 
        else:
    ...
    if quantize_lambda is not None and args.prune_algo is not None and args.prune_algo not in ["low_rank", "model_size_prune"]:
        f.write("---------------------quant-----------------------\r\n")
        prune_lambda(model)
        acc, acc_adv = model_test(model, 1, test_loader, 
            atk_algo=attack_algo, atk_eps=args.attack_eps, 
            iscuda=args.cuda, adv_iter=args.attack_iter, criterion=F.cross_entropy)
