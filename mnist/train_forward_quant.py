...
from util_trts import model_train, model_test, model_train_admm, model_train_proj_prune_admm_quant, model_train_together
import util_trts
...
from caffelenet.caffelenet_quantforward import CaffeLeNet as CLquant
...
elif args.quantize_algo == "forward_quant":
    quantize_algo = pt.quantize_bit_compute
else:
    quantize_algo = None

quantize_name = "None" if args.quantize_algo is None else args.quantize_algo

model_base = CLquant(args.quantize_bits)
...
if args.abc_special:
    ...
    if not args.lr_initialize:
        ...
        with open(os.path.join(args.logdir, "param_" + descripter + "lr.npy"), "rb") as filestream:
    ...
    model_base.load_state_dict(torch.load(model_path), strict=False)
    model = model_base
    modelu, modelz = CLquant(args.quantize_bits), CLquant(args.quantize_bits)
...
try:
    ...
    weight_bits = [math.ceil(math.log2(item-1)) for item in list(pt.layers_unique(model)[1].values())]
    model_size = 0
    ...
    for epoch in range(args.epochs):
        ...
        message = model_train_together(model, epoch, train_loader, optimizer,
        ...
        if epoch % args.test_interval == 0:
            ...
            if quantize_lambda is not None:
                ...
                layers = layers_unique(model, normalized=False, param_name=["weight_q"])[1]
