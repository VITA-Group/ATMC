...
def model_train_together(model, epoch, data_loader, optimizer, \
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        #         # print("what?")
        #         loss += rho / 2. * model.module.admm_regularizer(model_u, model_dual)
        #     else:
        #         # print("show?")
            ...
            with torch.no_grad():
                ...
                if quantize_tk is not None:
                    if batch_idx % admm_interval == 0 and batch_idx > 0:
                        if isinstance(model, nn.DataParallel):
                            # model_dual.module.duplicate_plus(model, model_u)
                            if nnzfix:
                                prune_tk(model)
                            if weight_bits is not None:
                                weight_bits, clusters = quantize_tk(list(param_list(model)), nnz, list(param_list(model)))
                            else:
                                quantize_tk(model)
                            # model_u.module.duplicate_update(model, model_dual, rho)
                            model.module.update_weight_bit(weight_bits)
                        else:
                            ...
                            model.update_weight_bit(weight_bits)
