...
def model_saver(model, root, model_name, descripter=""):
    if model_name is None:
        model_snapshot(model, os.path.join(root, descripter + "_densepretrain.pth"))
    else:
        model_snapshot(model, os.path.join(root, descripter + model_name))

def model_loader(root, model_name, descripter=""):
    return torch.load(os.path.join(root, descripter + model_name))
