...
class BottleneckSuper(nn.Module):
    ...
    def __init__(self, in_planes, planes, ConvF, ranks=None, stride=1):
        ...
        if ConvF is nn.Conv2d:
            ...
            self.conv1 = ConvF(in_planes, planes, rank=ranks[0], kernel_size=1, bias=False)
            self.rank_accumulator += 1
        self.bn1 = nn.BatchNorm2d(planes)
        if ConvF is nn.Conv2d:
            self.conv2 = ConvF(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        else:
            self.conv2 = ConvF(planes, planes, rank=ranks[1], kernel_size=3, stride=stride, padding=1, bias=False)
            self.rank_accumulator += 1
        self.bn2 = nn.BatchNorm2d(planes)
        if ConvF is nn.Conv2d:
            self.conv3 = ConvF(planes, self.expansion*planes, kernel_size=1, bias=False)
        else:
            self.conv3 = ConvF(planes, self.expansion*planes, rank=ranks[2], kernel_size=1, bias=False)
            self.rank_accumulator += 1
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            if ConvF is nn.Conv2d:
                ...
                self.shortcut = nn.Sequential(
                    ConvF(in_planes, self.expansion*planes, kernel_size=1, rank=ranks[4], stride=stride, bias=False),
                    nn.BatchNorm2d(self.expansion*planes)
                )
                self.rank_accumulator += 1
...
class ResNetSuper(nn.Module):
    offset = 0
    def __init__(self, block, num_blocks, ConvF, LinF, ranks=None, num_classes=100):
