...
import dataset
from effnets2.mobilenetv1 import MobileNet
from effnets2.mobilenetv2 import MobileNetV2
from effnets2.shufflenetv1 import ShuffleNet
from effnets2.shufflenetv2 import ShuffleNetV2
...
parser.add_argument('--model', default='MobileNet_shallow', help="which low cost model to use")
parser.add_argument('--alpha', type=float, default=1, help="alpha of kernel size. mobile net: w*alpha, h*alpha; shufflenet: c*alpha")
parser.add_argument('--groups', type=int, default=3, help="group num for shuffle net")
...
train_loader, test_loader = dataset.get100(batch_size=args.batch_size, data_root=args.data_root, num_workers=1)
...
if args.model == 'MobileNet':  
    model, modelu, modelz = MobileNet(alpha=args.alpha), MobileNet(alpha=args.alpha), MobileNet(alpha=args.alpha)
elif args.model == 'MobileNetV2':
    model, modelu, modelz = MobileNetV2(alpha=args.alpha), MobileNetV2(alpha=args.alpha), MobileNetV2(alpha=args.alpha)
elif args.model == 'ShuffleNet':  
    model, modelu, modelz = ShuffleNet(alpha=args.alpha, groups=args.groups), ShuffleNet(alpha=args.alpha, groups=args.groups), ShuffleNet(alpha=args.alpha, groups=args.groups)
elif args.model == 'ShuffleNetV2':  
    model, modelu, modelz = ShuffleNetV2(net_size=args.alpha), ShuffleNetV2(net_size=args.alpha), ShuffleNetV2(net_size=args.alpha)
else:
    raise Exception('Unimplemented Network Struscture!')
...
if args.raw_train:
    ...
    if args.model_name is None:
        ...
        model_path = os.path.join(args.loaddir, args.prefix_name + descripter + args.model_name)

# model definition is simple for effnets, just like dense models:
...
f.write('alpha: %f, groups: %d, attack: %s-%d-%d\n' % (args.alpha, args.groups, args.attack_algo, args.attack_eps, args.attack_iter))

try:
    import math
    # ready to go
    acc, acc_adv = model_test(model, 0, test_loader, 
    ...
    if quantize_algo is not None:
        if args.quantize_algo == "kmeans_nnz_fixed_0_center":
            weight_bits = [math.ceil(math.log2(item-1)) for item in list(pt.layers_unique(model, weight_name)[1].values())]
        elif args.quantize_algo == "kmeans":
            weight_bits = [math.ceil(math.log2(item)) for item in list(pt.layers_unique(model, weight_name)[1].values())]
        elif args.quantize_algo == "kmeans_fixed_nnz":
            weight_bits = [math.ceil(math.log2(item-1)) for item in list(pt.layers_unique(model, weight_name)[1].values())]
        else:
            raise 'unsupported quantize algo'
    else:
        weight_bits = [32] * len(layers)
    model_size = 0
    for i in range(len(layers)):
        model_size += list(layers.values())[i] * weight_bits[i]
    ...
    if quantize_lambda is not None and args.prune_algo is not None and args.prune_algo not in ["low_rank", "model_size_prune"]:
        f.write("-----------------------------------------------\r\n")
        prune_lambda(model)
        acc, acc_dv = model_test(model, 1, test_loader, 
            atk_algo=attack_algo, atk_eps=args.attack_eps, 
            iscuda=args.cuda, adv_iter=args.attack_iter, criterion=F.cross_entropy)
